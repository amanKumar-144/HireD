...
contract SimpleStorage {
  ...
  function registerSeeker(string memory name,string memory age,string memory gender,string memory emailId,
    string memory phoneNum) public {
      ...
      jobSeekerMap[jobSeekerCount] = JobSeeker(jobSeekerCount,name,age,gender,emailId,phoneNum,msg.sender);
  }
  ...
  string memory companyUrl,string memory location,string memory description) public {
    jobCount++;
  ...
  function acceptJobFromCompany(uint jobId,uint seekerId) public {
    for(uint i = 1;i <= totalJobsApplied ;i++){
